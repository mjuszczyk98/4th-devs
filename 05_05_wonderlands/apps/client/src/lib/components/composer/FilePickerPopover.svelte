...
type PickerIconType = IconType | 'folder'
  ...
  >
    ...
    >
      ...
      {:else if results.length === 0}
        <div class="px-3 py-3 text-center text-sm text-text-tertiary">
          {isEmptyQuery ? 'No files or folders in workspace' : 'No files or folders found'}
        </div>
      {:else}
        {#each groupedResults as group, groupIndex}
          ...
          {#each group.items as result, itemIndex}
            ...
            >
              ...
              {:else}
                {@const icon: PickerIconType = result.kind === 'directory' ? 'folder' : extToIcon(result.extension)}
                <svg class="mt-0.5 shrink-0 text-text-tertiary" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round">
                  {#if icon === 'folder'}
                    <path d="M2.5 4.5a1.5 1.5 0 0 1 1.5-1.5h2l1.2 1.5H12a1.5 1.5 0 0 1 1.5 1.5v5.5A1.5 1.5 0 0 1 12 13H4a1.5 1.5 0 0 1-1.5-1.5z" />
                    <path d="M2.5 6h11" />
                  {:else if icon === 'code'}
