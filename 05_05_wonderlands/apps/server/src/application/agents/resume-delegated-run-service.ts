...
import { resolveRunWait, type RunWaitResolutionState } from '../runtime/waits/run-wait-resolution'
...
export const resumeDelegatedRun = async (
  ...
  if (input.approve !== undefined) {
    ...
    if (toolExecution.ok && requiresConfirmationForToolWait(runDependency.value, toolExecution.value)) {
      return err({
