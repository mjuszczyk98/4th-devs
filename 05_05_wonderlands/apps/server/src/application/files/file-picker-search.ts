...
const toDirectoryRelativePath = (value: string): string => `${value}/`
    ...
    for (const child of children) {
      ...
      if (isDirectory) {
        entries.push({
          ...
          pathLower: toDirectoryRelativePath(relativePath).toLowerCase(),
          relativePath: toDirectoryRelativePath(relativePath),
          source: 'workspace',
        })
...
const toPathMatchStartIndex = (entry: IndexedEntry): number =>
  entry.source === 'workspace' && entry.kind === 'directory'
    ? entry.relativePath.length - entry.fileName.length - 1
    : entry.relativePath.length - entry.fileName.length

const mapNameIndicesToPathIndices = (entry: IndexedEntry, indices: readonly number[]): number[] => {
  const startIndex = toPathMatchStartIndex(entry)
