...
### Lista brakujących funkcjonalności w klientach MCP

Zaledwie pojedyncze klienty MCP deklarują pełne wsparcie protokołu. Przy wyborze / budowie interfejsu weryfikuj:

- Dostęp do historii wiadomości wielu użytkowników (np. kanały Slack)
- Tworzenie profili asystentów per obszar (np. projekty w Claude)
- Automatyczna konfiguracja ustawień (np. współdzielenie skilli w Claude Code)
- Elastyczny interfejs (np. Artifacts w ChatGPT)
- Opcja korzystania z różnych modeli (np. Raycast)
- Przetwarzanie różnych formatów treści (np. źródła w NotebookLM)
- Interakcje audio & wideo (np. tryb live w Google AI Studio)
- Dostęp do funkcji systemowych (aplikacje desktopowe)
- Pełna prywatność (np. LM Studio)

### Szczegółowa struktura meta-promptu — opisy sekcji

| Sekcja | Opis |
|---|---|
| **Proces** | Model informowany, że cel = przeprowadzenie użytkownika przez serię pytań w celu ukształtowania instrukcji |
| **Strategia** | Punkty opisujące realizację celu z dopasowaniem do bieżącej sytuacji |
| **Dopasowanie** | Zasady specjalizacji pod różne dziedziny + obsługa wymagań wykraczających poza domyślną strukturę |
| **Format** | Szablon końcowej instrukcji + sposób prezentacji (np. blok markdown z opcją „instalacji") |
| **Zasady** | Reguły prowadzenia rozmowy: oczekiwanie na odpowiedź, pogłębianie niejasności, **model nie zgaduje** |
| **Natywne funkcjonalności** | Generowanie nie tylko instrukcji, ale też ustawień agenta (model, tryby, integracje) jako wartości domyślne |
| **Proces generowania** | Selekcja — brać pod uwagę wyłącznie elementy istotne dla bieżącego kontekstu |
| **Krytyczne zasady** | Podkreślenie najważniejszych reguł na końcu instrukcji; część reguł powinna pojawiać się w metadanych wiadomości |

### Konkretne przykłady zastosowań meta-promptów

- **Onboarding marketplace:** użytkownik dodaje produkty → z opisów, strony produktu i zdjęć automatycznie generowane są instrukcje do personalizacji (np. generowanie opisów produktów, kreacji reklamowych dopasowanych do tonu marki)
- **Generowanie obrazu:** meta-prompt generuje instrukcję edycji obrazu dopasowaną do konkretnego produktu i tonu marki — nie tylko generyczny prompt
- **Optymalizacja promptów:** nawet proste agenty (np. klasyfikacja danych) mogą korzystać ze strategii automatycznej optymalizacji instrukcji opartych na meta-promptach

### Mikro-akcje — brakujący wpis

| Typ akcji | Przykład |
|---|---|
| Lokalizacja zdjęcia | Zdjęcie z telefonu + metadane (GPS) → kontekstowa akcja (np. dopisanie do listy zakupów) |
